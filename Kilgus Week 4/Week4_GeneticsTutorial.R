...
# To look at genotypic evenness we can use a variation of our old friend the histogram.
  # This time we are comparing histograms for the hight (count) of dominant bars (richness) rather than the best distribution model.
mon.tab <- mlg.table(monpop)
    ...
    # Do you think the genotypes are more "even" in the BB or the FR population?
        ...
        # If you said BB, then you are correct! The most abundant genotypes in BB are still <1/4 as dominatant as for FR.
...
plot(p, interactive=TRUE)
  # Now click on a node, move your cursur to another location, and move again.
  # This might take a few seconds depnding on which node is moved and how far (watch the "STOP" button in the corner of the console)
...
# These data are from: https://datadryad.org/stash/dataset/doi:10.5061/dryad.fb436.
  ...
  # For context, there are some great maps of these allele frequencies in their paper...even if they do include pie charts.
    # The paper is uploaded to Blackboard in this folder under "Jaramillo-Correa et al. - Molecular proxies and pines"
  
# Download the .csv file from Blackboard and save in your desired working directory.
  # Make sure to set your working directory then read the data into R:
#setwd("C:/Users/wilsonmatt/OneDrive - Susquehanna University/Courses/Ecological Data Analysis/Week 4 - Genetics")
...
summary(mod.lon)
  # The significant intercept is NOT important for the relationship of our variables. It just means the intercept isn't zero.
...
# Additionally, if you want to view how sample locations relate to each other, you can use a scatterplot pretending one predicts the other:
...
summary(mod.fac)
 # Now we can see that a couple very specific regions have wildly different genotype means from the rest.
    # Particularly Soria. These significan regions are what likely drives the significant Lan/Lon relationship.
